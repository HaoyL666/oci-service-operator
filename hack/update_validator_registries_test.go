...
func TestScanConfiguredAPISpecsSkipsBlockedServicesWithoutGeneratedAPIs(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	writeValidatorRegistryConfig(t, root, `
  - service: opensearch
    sdkPackage: github.com/oracle/oci-go-sdk/v65/opensearch
    group: opensearch
    ...
    generation:
      ...
      resources:
        - kind: OpensearchOpensearchCluster
          formalSpec: opensearchopensearchcluster
          promotionBlockers:
            - oci-service-operator-fk9.6
`)
	...
	if err != nil {
		t.Fatalf("scanConfiguredAPISpecs() error = %v", err)
	}
	if len(apiSpecs) != 0 {
		t.Fatalf("len(apiSpecs) = %d, want 0 for blocked service without generated APIs", len(apiSpecs))
	}
}

func TestScanConfiguredAPISpecsRejectsUnblockedServicesWithoutGeneratedAPIs(t *testing.T) {
	t.Parallel()
...
`)
	...
	_, err = scanConfiguredAPISpecs(root, services)
	if err == nil {
		t.Fatal("scanConfiguredAPISpecs() error = nil, want missing API specs failure")
	}
	if want := `configured API group "opensearch" has no spec types under api/opensearch/v1beta1`; err.Error() != want {
		t.Fatalf("scanConfiguredAPISpecs() error = %q, want %q", err.Error(), want)
	}
}
...
func TestDeriveSDKTypes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "compatibility locked kind uses stripped sdk base",
			service: "opensearch",
			group:   "opensearch",
			spec:    "OpensearchOpensearchCluster",
			structs: []string{"CreateOpensearchClusterDetails", "UpdateOpensearchClusterDetails", "OpensearchCluster", "OpensearchClusterSummary", "OpensearchClusterCollection"},
			want:    []string{"CreateOpensearchClusterDetails", "UpdateOpensearchClusterDetails", "OpensearchCluster", "OpensearchClusterSummary", "OpensearchClusterCollection"},
		},
		{
			name:    "compatibility locked version uses explicit plural override",
			service: "opensearch",
			group:   "opensearch",
			spec:    "OpensearchOpensearchVersion",
			structs: []string{"OpensearchVersionsSummary", "OpensearchVersionsCollection"},
			want:    []string{"OpensearchVersionsSummary", "OpensearchVersionsCollection"},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := deriveSDKTypes(tt.service, tt.group, tt.spec, makeTargetName(tt.group, tt.spec), structs)
...
func TestMakeTargetNameAvoidsDuplicatePrefix(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:  "compatibility mysql kind",
			group: "mysql",
			spec:  "MySqlDbSystem",
			want:  "MySqlDbSystem",
		},
		{
			name:  "compatibility object storage kind",
			group: "objectstorage",
			spec:  "ObjectStorageBucket",
			want:  "ObjectStorageBucket",
		},
		{
			name:  "compatibility opensearch kind",
			group: "opensearch",
			spec:  "OpensearchOpensearchCluster",
			want:  "OpensearchOpensearchCluster",
		},
		{
			name:  "unprefixed opensearch kind",
			group: "opensearch",
			spec:  "Manifest",
			want:  "OpensearchManifest",
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := makeTargetName(tt.group, tt.spec); got != tt.want {
				t.Fatalf("makeTargetName(%q, %q) = %q, want %q", tt.group, tt.spec, got, tt.want)
			}
		})
	}
}

func TestResolvedTargetNameRefreshesLegacyAutoGeneratedNames(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:         "no existing name keeps current target",
			group:        "opensearch",
			spec:         "OpensearchOpensearchCluster",
			targetName:   "OpensearchOpensearchCluster",
			hasExisting:  false,
			existingName: "",
			want:         "OpensearchOpensearchCluster",
		},
		{
			name:         "legacy auto generated name is refreshed",
			group:        "opensearch",
			spec:         "OpensearchOpensearchCluster",
			targetName:   "OpensearchOpensearchCluster",
			hasExisting:  true,
			existingName: "OpensearchOpensearchOpensearchCluster",
			want:         "OpensearchOpensearchCluster",
		},
		{
			name:         "handwritten existing name is preserved",
			group:        "core",
			spec:         "ConsoleHistoryContent",
			targetName:   "CoreConsoleHistoryContent",
			hasExisting:  true,
			existingName: "ConsoleHistoryContent",
			want:         "ConsoleHistoryContent",
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := resolvedTargetName(tt.group, tt.spec, tt.targetName, tt.hasExisting, tt.existingName); got != tt.want {
				t.Fatalf("resolvedTargetName(%q, %q, %q, %v, %q) = %q, want %q", tt.group, tt.spec, tt.targetName, tt.hasExisting, tt.existingName, got, tt.want)
			}
		})
	}
}
...
func TestBuildSDKMappingsOpensearchOverrides(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name:       "cluster excludes collection",
			spec:       "OpensearchOpensearchCluster",
			candidates: []string{"CreateOpensearchClusterDetails", "UpdateOpensearchClusterDetails", "OpensearchCluster", "OpensearchClusterCollection", "OpensearchClusterSummary"},
			want: map[string]sdkMapping{
				...
				"opensearch.OpensearchClusterCollection": {
		...
		{
			name:       "version uses status summary and excludes collection",
			spec:       "OpensearchOpensearchVersion",
			candidates: []string{"OpensearchVersionsSummary", "OpensearchVersionsCollection"},
			want: map[string]sdkMapping{
				"opensearch.OpensearchVersionsSummary": {
					SDKStruct:  "opensearch.OpensearchVersionsSummary",
					APISurface: "status",
				},
				"opensearch.OpensearchVersionsCollection": {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := buildSDKMappings("opensearch", tt.spec, tt.candidates, false, specTarget{})
			assertExactMappings(t, got, tt.want)
		})
	}
}
...
func writeValidatorRegistryConfig(t *testing.T, root string, servicesYAML string) {
	t.Helper()
	...
	content := `schemaVersion: v1alpha1
domain: oracle.com
...
services:
` + servicesYAML
	if err := os.WriteFile(filepath.Join(configDir, "services.yaml"), []byte(content), 0o644); err != nil {
		t.Fatalf("WriteFile(services.yaml) error = %v", err)
	}
}
