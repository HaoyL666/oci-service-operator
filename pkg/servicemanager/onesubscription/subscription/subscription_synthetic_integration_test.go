...
func TestSyntheticOneSubscriptionReadsExisting(t *testing.T) {
	...
	metadata := ocireplay.Metadata{Service: "onesubscription", Resource: "Subscription", Operations: []ocireplay.Operation{ocireplay.OperationRead}, SDKVersion: "v65.110.0", Provenance: ocireplay.ProvenanceSynthetic}
	session, err := ocireplay.OpenSDKSynthetic(ocireplay.SDKSyntheticOptions{Path: filepath.Join("testdata", "recordings", "subscription_synthetic_read.yaml"), Host: "https://onesubscription.us-ashburn-1.oci.oraclecloud.com", BasePath: "20190111", Metadata: metadata, Responses: []ocireplay.SyntheticResponse{{StatusCode: http.StatusOK, Body: observed}}})
	if err != nil {
		t.Fatal(err)
	}
	defer func() {
	...
	sdkClient := onesubscriptionsdk.SubscriptionClient{BaseClient: session.BaseClient()}
