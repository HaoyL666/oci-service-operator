...
func (c manualDbSystemServiceClient) CreateOrUpdate(
	...
	if !found {
		createDetails, err := buildCreateDbSystemDetails(resource.Spec)
		if err != nil {
			return servicemanager.OSOKResponse{IsSuccessful: false}, c.markFailure(resource, err)
		}
...
func (c manualDbSystemServiceClient) Delete(
	...
	if _, err := c.projectDbSystem(resource, refreshed, shared.Terminating); err != nil {
		return false, c.markFailure(resource, err)
	}
...
func buildCreateDbSystemDetails(spec psqlv1beta1.DbSystemSpec) (psqlsdk.CreateDbSystemDetails, error) {
	...
	if isZeroValue(spec.Credentials) {
