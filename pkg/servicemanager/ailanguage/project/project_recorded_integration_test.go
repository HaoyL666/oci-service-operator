...
const recordedProjectName = "osok-replay-ai-language-project-v1"
...
func TestRecordedProjectCreateUpdateDelete(t *testing.T) {
	...
	resource := &ailanguagev1beta1.Project{Spec: ailanguagev1beta1.ProjectSpec{
		CompartmentId: compartmentID, DisplayName: recordedProjectName,
		FreeformTags: map[string]string{"osok-replay": "create"},
	}}
	client := newProjectServiceClientWithOCIClient(loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("recorded-integration")}, sdkClient)
	...
	resource.Spec.DisplayName = recordedProjectName + "-updated"
...
func openRecordedProjectSDK(t *testing.T, mode ocireplay.Mode) (ailanguagesdk.AIServiceLanguageClient, func() error) {
	t.Helper()
	metadata := ocireplay.Metadata{Service: "ailanguage", Resource: "Project", Operations: []ocireplay.Operation{ocireplay.OperationCreate, ocireplay.OperationRead, ocireplay.OperationUpdate, ocireplay.OperationDelete}, SDKVersion: "v65.110.0", Provenance: ocireplay.ProvenanceRecorded}
	...
	session, err := ocireplay.OpenSDKReplay(ocireplay.SDKReplayOptions{Path: path, Host: "https://language.aiservice.us-ashburn-1.oci.oraclecloud.com", BasePath: "20221001", Metadata: metadata})
	if err != nil {
		t.Fatal(err)
	}
	return ailanguagesdk.AIServiceLanguageClient{BaseClient: session.BaseClient()}, session.Close
}

func awaitRecordedProject(ctx context.Context, mode ocireplay.Mode, client ProjectServiceClient, resource *ailanguagev1beta1.Project) error {
