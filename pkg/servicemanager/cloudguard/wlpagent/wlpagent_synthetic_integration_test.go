...
func TestSyntheticWlpAgentCreateReadDelete(t *testing.T) {
	resource := &cloudguardv1beta1.WlpAgent{}
	if err := json.Unmarshal([]byte(`{"metadata":{"name":"osok-replay-wlpagent"},"spec":{"compartmentId":"ocid1.compartment.oc1..synthetic","agentVersion":"1.0.0","certificateSignedRequest":"synthetic-csr","osInfo":"linux_amd64"}}`), resource); err != nil {
		t.Fatal(err)
	}
	createdBody, err := ocireplay.SyntheticObservedBody(resource.Spec, "ocid1.wlpagent.oc1..synthetic", "ACTIVE", nil)
	if err != nil {
		t.Fatal(err)
	}
	metadata := ocireplay.Metadata{
		Service: "cloudguard", Resource: "WlpAgent",
	...
	session, err := ocireplay.OpenSDKSynthetic(ocireplay.SDKSyntheticOptions{
		Path: filepath.Join("testdata", "recordings", "wlpagent_synthetic_crud.yaml"), Host: "https://cloudguard-cp-api.us-ashburn-1.oci.oraclecloud.com", BasePath: "20200131", Metadata: metadata,
	...
	manager := &WlpAgentServiceManager{Log: loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("synthetic-integration")}}
	...
	ocireplay.RunLifecycle(t, ocireplay.LifecycleScenario[*cloudguardv1beta1.WlpAgent]{
		...
		HasIdentity:   func(current *cloudguardv1beta1.WlpAgent) bool { return current.Status.OsokStatus.Ocid != "" },
		ValidateCreated: func(current *cloudguardv1beta1.WlpAgent) error {
			if current.Status.OsokStatus.Ocid == "" {
				return fmt.Errorf("created WlpAgent status has no OCI identity: %+v", current.Status)
			}
			return nil
		},
	})
