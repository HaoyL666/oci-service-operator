...
const recordedSecurityPolicyName = "osok-replay-security-policy-v1"

func TestRecordedSecurityPolicyCreateUpdateDelete(t *testing.T) {
	...
	if mode == ocireplay.ModeRecord {
		compartmentID = requiredSecurityPolicyRecordingEnv(t, "OCI_COMPARTMENT_ID")
	}
	sdkClient, closeSession := openRecordedSecurityPolicySDK(t, mode)
	resource := &datasafev1beta1.SecurityPolicy{
		ObjectMeta: metav1.ObjectMeta{Name: "recorded-security-policy-v1"},
		Spec: datasafev1beta1.SecurityPolicySpec{
			CompartmentId: compartmentID,
			DisplayName:   recordedSecurityPolicyName,
			Description:   "recorded create",
			FreeformTags:  map[string]string{"osok-replay": "create"},
		},
	}
	manager := &SecurityPolicyServiceManager{Log: loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("recorded-integration")}}
	...
	ocireplay.RunLifecycle(t, ocireplay.LifecycleScenario[*datasafev1beta1.SecurityPolicy]{
		...
		HasIdentity:   func(current *datasafev1beta1.SecurityPolicy) bool { return current.Status.OsokStatus.Ocid != "" },
		ValidateCreated: func(current *datasafev1beta1.SecurityPolicy) error {
			if current.Status.OsokStatus.Ocid == "" || current.Status.DisplayName != recordedSecurityPolicyName {
				return fmt.Errorf("created SecurityPolicy status = %+v", current.Status)
			}
			return nil
		},
		Mutate: func(current *datasafev1beta1.SecurityPolicy) {
...
func openRecordedSecurityPolicySDK(t *testing.T, mode ocireplay.Mode) (datasafesdk.DataSafeClient, func() error) {
	t.Helper()
	metadata := ocireplay.Metadata{
		Service: "datasafe", Resource: "SecurityPolicy",
	...
	path := filepath.Join("testdata", "recordings", "securitypolicy_crud.yaml")
	if mode == ocireplay.ModeRecord {
...
func requiredSecurityPolicyRecordingEnv(t *testing.T, name string) string {
	t.Helper()
