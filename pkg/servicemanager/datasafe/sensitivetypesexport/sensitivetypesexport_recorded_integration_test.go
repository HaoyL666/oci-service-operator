...
const recordedSensitiveTypesExportName = "osok-replay-sensitive-types-export-v1"

func TestRecordedSensitiveTypesExportCreateUpdateDelete(t *testing.T) {
	mode, err := ocireplay.RequestedMode()
	if err != nil {
		t.Fatal(err)
	}
	sdkClient, closeSession := openRecordedSensitiveTypesExportSDK(t, mode)
	compartmentID := "ocid1.compartment.oc1..replay"
	if mode == ocireplay.ModeRecord {
		compartmentID = requiredSensitiveTypesExportRecordingEnv(t, "OCI_COMPARTMENT_ID")
	}
	resource := &datasafev1beta1.SensitiveTypesExport{
		ObjectMeta: metav1.ObjectMeta{Name: "recorded-sensitive-types-export-v1"},
		Spec: datasafev1beta1.SensitiveTypesExportSpec{
			CompartmentId:              compartmentID,
			DisplayName:                recordedSensitiveTypesExportName,
			Description:                "recorded create",
			IsIncludeAllSensitiveTypes: true,
			FreeformTags:               map[string]string{"osok-replay": "create"},
		},
	}
	manager := &SensitiveTypesExportServiceManager{Log: loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("recorded-integration")}}
	...
	ocireplay.RunLifecycle(t, ocireplay.LifecycleScenario[*datasafev1beta1.SensitiveTypesExport]{
		...
		HasIdentity:   func(current *datasafev1beta1.SensitiveTypesExport) bool { return current.Status.OsokStatus.Ocid != "" },
		ValidateCreated: func(current *datasafev1beta1.SensitiveTypesExport) error {
			if current.Status.OsokStatus.Ocid == "" || current.Status.DisplayName != recordedSensitiveTypesExportName {
...
func openRecordedSensitiveTypesExportSDK(t *testing.T, mode ocireplay.Mode) (datasafesdk.DataSafeClient, func() error) {
	t.Helper()
	metadata := ocireplay.Metadata{
		Service: "datasafe", Resource: "SensitiveTypesExport",
	...
	path := filepath.Join("testdata", "recordings", "sensitivetypesexport_crud.yaml")
	if mode == ocireplay.ModeRecord {
...
func requiredSensitiveTypesExportRecordingEnv(t *testing.T, name string) string {
	t.Helper()
