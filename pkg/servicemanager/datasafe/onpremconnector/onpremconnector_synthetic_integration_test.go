...
func TestSyntheticOnPremConnectorCreateReadDelete(t *testing.T) {
	resource := &datasafev1beta1.OnPremConnector{}
	if err := json.Unmarshal([]byte(`{"metadata":{"name":"osok-replay-onpremconnector"},"spec":{"compartmentId":"ocid1.compartment.oc1..synthetic","displayName":"osok-replay-onprem-connector"}}`), resource); err != nil {
		t.Fatal(err)
	}
	createdBody, err := ocireplay.SyntheticObservedBody(resource.Spec, "ocid1.onpremconnector.oc1..synthetic", "ACTIVE", nil)
	if err != nil {
		t.Fatal(err)
	}
	metadata := ocireplay.Metadata{
		Service: "datasafe", Resource: "OnPremConnector",
	...
	session, err := ocireplay.OpenSDKSynthetic(ocireplay.SDKSyntheticOptions{
		Path: filepath.Join("testdata", "recordings", "onpremconnector_synthetic_crud.yaml"), Host: "https://datasafe.us-ashburn-1.oci.oraclecloud.com", BasePath: "20181201", Metadata: metadata,
	...
	ocireplay.RunLifecycle(t, ocireplay.LifecycleScenario[*datasafev1beta1.OnPremConnector]{
		...
		HasIdentity:   func(current *datasafev1beta1.OnPremConnector) bool { return current.Status.OsokStatus.Ocid != "" },
		ValidateCreated: func(current *datasafev1beta1.OnPremConnector) error {
			if current.Status.OsokStatus.Ocid == "" {
				return fmt.Errorf("created OnPremConnector status has no OCI identity: %+v", current.Status)
			}
			return nil
		},
	})
