/*
  Copyright (c) 2026, Oracle and/or its affiliates. All rights reserved.
  Licensed under the Universal Permissive License v 1.0 as shown at http://oss.oracle.com/licenses/upl.
*/

package filesystem

import (
	"context"
	"fmt"
	"os"
	"path/filepath"
	"testing"
	"time"

	filestoragesdk "github.com/oracle/oci-go-sdk/v65/filestorage"
	filestoragev1beta1 "github.com/oracle/oci-service-operator/api/filestorage/v1beta1"
	"github.com/oracle/oci-service-operator/internal/e2e/ocireplay"
	"github.com/oracle/oci-service-operator/pkg/loggerutil"
	generatedruntime "github.com/oracle/oci-service-operator/pkg/servicemanager/generatedruntime"
	ctrl "sigs.k8s.io/controller-runtime"
)

const recordedFileSystemName = "osok-replay-async-fs-v1"

func TestRecordedFileSystemCreateUpdateDelete(t *testing.T) {
	mode, err := ocireplay.RequestedMode()
	if err != nil {
		t.Fatal(err)
	}

	sdkClient, closeSession := openRecordedFileSystemSDK(t, mode)
	closed := false
	t.Cleanup(func() {
		if !closed && mode == ocireplay.ModeReplay {
			if err := closeSession(); err != nil {
				t.Errorf("close FileSystem cassette: %v", err)
			}
		}
	})

	compartmentID := "ocid1.compartment.oc1..replay"
	availabilityDomain := "example:AD-1"
	if mode == ocireplay.ModeRecord {
		compartmentID = requiredFileSystemRecordingEnv(t, "OCI_COMPARTMENT_ID")
		availabilityDomain = requiredFileSystemRecordingEnv(t, "OCI_AVAILABILITY_DOMAIN")
	}

	resource := &filestoragev1beta1.FileSystem{
		Spec: filestoragev1beta1.FileSystemSpec{
			AvailabilityDomain: availabilityDomain,
			CompartmentId:      compartmentID,
			DisplayName:        recordedFileSystemName,
			FreeformTags: map[string]string{
				"osok-replay": "create",
			},
		},
	}
	manager := &FileSystemServiceManager{
		Log: loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("recorded-integration")},
	}
	hooks := newFileSystemDefaultRuntimeHooks(sdkClient)
	client := defaultFileSystemServiceClient{
		ServiceClient: generatedruntime.NewServiceClient[*filestoragev1beta1.FileSystem](
			buildFileSystemGeneratedRuntimeConfig(manager, hooks),
		),
	}

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Minute)
	defer cancel()

	deleted := false
	if mode == ocireplay.ModeRecord {
		t.Cleanup(func() {
			if deleted {
				return
			}
			cleanupCtx, cleanupCancel := context.WithTimeout(context.Background(), 10*time.Minute)
			defer cleanupCancel()
			_ = ocireplay.Await(cleanupCtx, mode, 5*time.Second, func() (bool, error) {
				return client.Delete(cleanupCtx, resource)
			})
		})
	}

	createCtx := generatedruntime.WithSkipExistingBeforeCreate(ctx)
	if err := awaitFileSystemConvergence(createCtx, mode, client, resource); err != nil {
		t.Fatal(err)
	}
	if resource.Status.LifecycleState != string(filestoragesdk.FileSystemLifecycleStateActive) ||
		resource.Status.DisplayName != recordedFileSystemName {
		t.Fatalf("created FileSystem status = %+v", resource.Status)
	}

	resource.Spec.DisplayName = recordedFileSystemName + "-updated"
	resource.Spec.FreeformTags = map[string]string{"osok-replay": "update"}
	if err := awaitFileSystemConvergence(ctx, mode, client, resource); err != nil {
		t.Fatal(err)
	}
	if resource.Status.DisplayName != resource.Spec.DisplayName ||
		resource.Status.FreeformTags["osok-replay"] != "update" {
		t.Fatalf("updated FileSystem status = %+v", resource.Status)
	}

	if err := ocireplay.Await(ctx, mode, 5*time.Second, func() (bool, error) {
		return client.Delete(ctx, resource)
	}); err != nil {
		t.Fatal(err)
	}
	deleted = true

	if err := closeSession(); err != nil {
		t.Fatal(err)
	}
	closed = true
}

func openRecordedFileSystemSDK(
	t *testing.T,
	mode ocireplay.Mode,
) (filestoragesdk.FileStorageClient, func() error) {
	t.Helper()

	metadata := ocireplay.Metadata{
		Service:  "filestorage",
		Resource: "FileSystem",
		Operations: []ocireplay.Operation{
			ocireplay.OperationCreate,
			ocireplay.OperationRead,
			ocireplay.OperationUpdate,
			ocireplay.OperationDelete,
		},
		SDKVersion: "v65.110.0",
		Provenance: ocireplay.ProvenanceRecorded,
	}
	cassettePath := filepath.Join("testdata", "recordings", "filesystem_crud.yaml")

	if mode == ocireplay.ModeRecord {
		provider, err := ocireplay.RecordingConfigurationProvider()
		if err != nil {
			t.Fatal(err)
		}
		client, err := filestoragesdk.NewFileStorageClientWithConfigurationProvider(provider)
		if err != nil {
			t.Fatal(err)
		}
		session, err := ocireplay.OpenSDKRecord(ocireplay.SDKRecordOptions{
			Path:       cassettePath,
			Metadata:   metadata,
			BaseClient: &client.BaseClient,
			Bindings: map[string]string{
				"availability-domain": requiredFileSystemRecordingEnv(t, "OCI_AVAILABILITY_DOMAIN"),
			},
			Overwrite: ocireplay.RecordingOverwriteRequested(),
		})
		if err != nil {
			t.Fatal(err)
		}
		return client, session.Close
	}

	session, err := ocireplay.OpenSDKReplay(ocireplay.SDKReplayOptions{
		Path:     cassettePath,
		Host:     "https://filestorage.us-ashburn-1.oci.oraclecloud.com",
		BasePath: "20171215",
		Metadata: metadata,
		Bindings: map[string]string{
			"availability-domain": "example:AD-1",
		},
	})
	if err != nil {
		t.Fatal(err)
	}
	return filestoragesdk.FileStorageClient{
		BaseClient: session.BaseClient(),
	}, session.Close
}

func awaitFileSystemConvergence(
	ctx context.Context,
	mode ocireplay.Mode,
	client FileSystemServiceClient,
	resource *filestoragev1beta1.FileSystem,
) error {
	return ocireplay.Await(ctx, mode, 5*time.Second, func() (bool, error) {
		response, err := client.CreateOrUpdate(ctx, resource, ctrl.Request{})
		if err != nil {
			return false, err
		}
		if !response.IsSuccessful {
			return false, fmt.Errorf("FileSystem reconciliation was unsuccessful: %+v", response)
		}
		return !response.ShouldRequeue, nil
	})
}

func requiredFileSystemRecordingEnv(t *testing.T, name string) string {
	t.Helper()
	value := os.Getenv(name)
	if value == "" {
		t.Fatalf("%s is required in record mode", name)
	}
	return value
}
