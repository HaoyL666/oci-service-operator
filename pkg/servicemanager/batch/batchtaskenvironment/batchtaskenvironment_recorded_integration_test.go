...
const recordedBatchTaskEnvironmentName = "osok-replay-batch-task-environment-v1"

func TestRecordedBatchTaskEnvironmentCreateUpdateDelete(t *testing.T) {
	mode, err := ocireplay.RequestedMode()
	if err != nil {
		t.Fatal(err)
	}
	compartmentID := "ocid1.compartment.oc1..replay"
	imageURL := "iad.ocir.io/example/osok:replay"
	if mode == ocireplay.ModeRecord {
		compartmentID = requiredBatchTaskEnvironmentRecordingEnv(t, "OCI_COMPARTMENT_ID")
		imageURL = requiredBatchTaskEnvironmentRecordingEnv(t, "OCI_REPLAY_BATCH_IMAGE_URL")
	}
	sdkClient, closeSession := openRecordedBatchTaskEnvironmentSDK(t, mode, imageURL)
	resource := &batchv1beta1.BatchTaskEnvironment{
		ObjectMeta: metav1.ObjectMeta{Name: "recorded-batch-task-environment-v1"},
		Spec: batchv1beta1.BatchTaskEnvironmentSpec{
			CompartmentId: compartmentID,
			ImageUrl:      imageURL,
			DisplayName:   recordedBatchTaskEnvironmentName,
			Description:   "recorded create",
			FreeformTags:  map[string]string{"osok-replay": "create"},
		},
	}
	manager := &BatchTaskEnvironmentServiceManager{Log: loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("recorded-integration")}}
	...
	ocireplay.RunLifecycle(t, ocireplay.LifecycleScenario[*batchv1beta1.BatchTaskEnvironment]{
		...
		ValidateCreated: func(current *batchv1beta1.BatchTaskEnvironment) error {
			if current.Status.OsokStatus.Ocid == "" || current.Status.DisplayName != recordedBatchTaskEnvironmentName || current.Status.ImageUrl != imageURL {
				return fmt.Errorf("created BatchTaskEnvironment status = %+v", current.Status)
			}
			return nil
		},
		Mutate: func(current *batchv1beta1.BatchTaskEnvironment) {
...
func openRecordedBatchTaskEnvironmentSDK(t *testing.T, mode ocireplay.Mode, imageURL string) (batchsdk.BatchComputingClient, func() error) {
	t.Helper()
	...
	path := filepath.Join("testdata", "recordings", "batchtaskenvironment_crud.yaml")
	bindings := map[string]string{"batch-image-url": imageURL}
	if mode == ocireplay.ModeRecord {
	...
	session, err := ocireplay.OpenSDKReplay(ocireplay.SDKReplayOptions{Path: path, Host: "https://batch.us-ashburn-1.oci.oraclecloud.com", BasePath: "20251031", Metadata: metadata, Bindings: bindings})
	if err != nil {
		t.Fatal(err)
	}
...
func requiredBatchTaskEnvironmentRecordingEnv(t *testing.T, name string) string {
	t.Helper()
