...
func TestSyntheticPatchReconcilesTracked(t *testing.T) {
	resource := &fleetappsmanagementv1beta1.Patch{}
	...
	metadata := ocireplay.Metadata{Service: "fleetappsmanagement", Resource: "Patch", Operations: []ocireplay.Operation{ocireplay.OperationRead, ocireplay.OperationUpdate}, SDKVersion: "v65.110.0", Provenance: ocireplay.ProvenanceSynthetic}
	session, err := ocireplay.OpenSDKSynthetic(ocireplay.SDKSyntheticOptions{Path: filepath.Join("testdata", "recordings", "patch_synthetic_reconcile.yaml"), Host: "https://fams.us-ashburn-1.oci.oraclecloud.com", BasePath: "20250228", Metadata: metadata, Responses: []ocireplay.SyntheticResponse{{StatusCode: http.StatusOK, Body: observedBody}, {StatusCode: http.StatusOK, Body: observedBody}, {StatusCode: http.StatusOK, Body: observedBody}}})
	if err != nil {
		t.Fatal(err)
	}
	...
	client := wrapPatchGeneratedClient(hooks, defaultPatchServiceClient{ServiceClient: generatedruntime.NewServiceClient[*fleetappsmanagementv1beta1.Patch](buildPatchGeneratedRuntimeConfig(manager, hooks))})
