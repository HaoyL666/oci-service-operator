...
func TestSyntheticFleetCredentialReconcilesTracked(t *testing.T) {
	resource := &fleetappsmanagementv1beta1.FleetCredential{}
	resourceID := "ocid1.fleetcredential.oc1..synthetic"
	...
	metadata := ocireplay.Metadata{Service: "fleetappsmanagement", Resource: "FleetCredential", Operations: []ocireplay.Operation{ocireplay.OperationRead, ocireplay.OperationUpdate}, SDKVersion: "v65.110.0", Provenance: ocireplay.ProvenanceSynthetic}
	session, err := ocireplay.OpenSDKSynthetic(ocireplay.SDKSyntheticOptions{Path: filepath.Join("testdata", "recordings", "fleetcredential_synthetic_reconcile.yaml"), Host: "https://fams.us-ashburn-1.oci.oraclecloud.com", BasePath: "20250228", Metadata: metadata, Responses: []ocireplay.SyntheticResponse{{StatusCode: http.StatusOK, Body: observedBody}, {StatusCode: http.StatusOK, Body: observedBody}, {StatusCode: http.StatusOK, Body: observedBody}}})
	if err != nil {
		t.Fatal(err)
	}
	sdkClient := fleetappsmanagementsdk.FleetAppsManagementClient{BaseClient: session.BaseClient()}
	manager := &FleetCredentialServiceManager{Log: loggerutil.OSOKLogger{Logger: ctrl.Log.WithName("synthetic-integration")}}
	...
	client := wrapFleetCredentialGeneratedClient(hooks, defaultFleetCredentialServiceClient{ServiceClient: generatedruntime.NewServiceClient[*fleetappsmanagementv1beta1.FleetCredential](buildFleetCredentialGeneratedRuntimeConfig(manager, hooks))})
	response, err := client.CreateOrUpdate(context.Background(), resource, ctrl.Request{})
	if err != nil {
		t.Fatal(err)
	}
	if !response.IsSuccessful || resource.Status.OsokStatus.Ocid == "" {
		t.Fatalf("tracked FleetCredential response=%+v status=%+v", response, resource.Status)
	}
	if err := session.Close(); err != nil {
		t.Fatal(fmt.Errorf("close synthetic FleetCredential cassette: %w", err))
	}
}
