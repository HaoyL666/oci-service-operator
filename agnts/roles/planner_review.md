...
- [ ] The plan does not add controller/registration tasks unnecessarily when an existing generated group already delegates through those layers
- [ ] When a `formal/` row exists for the resource, the plan uses it as runtime context for service-manager work
- [ ] Field-scope decisions are grounded in the vendored SDK and checked-in repo contracts, not a newer external SDK copy unless the task is explicitly an SDK upgrade
...
- Reference examples: "See how `streams/` or `mysql/dbsystem/` separate client from manager"
...
Reference pattern: <existing service used as template, e.g. streams/, mysql/dbsystem/, or core/subnet/>
...
## Key Patterns
- <Pattern 1 the coder must follow, e.g. 'Service-manager client seam for testability'>
...
- ✅ Good: "Implement service manager in `pkg/servicemanager/queue/queue/queue_servicemanager.go` following the pattern in `pkg/servicemanager/streams/stream_servicemanager.go`. Must implement `CreateOrUpdate`, `Delete`, `GetCrdStatus` methods. Preserve the injectable client seam for testability."
