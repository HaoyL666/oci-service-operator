...
**CRD Types** (`api/<group>/v1beta1/*_types.go`):
...
**Controllers** (`controllers/<group>/*_controller.go`):
...
**Service Manager** (`pkg/servicemanager/<group>/<resource>/`):
- [ ] Implements the service client interface
- [ ] Has an injectable client seam that matches the package pattern (`ociClient` in older handwritten managers, `client` plus `WithClient(...)` in generated managers)
