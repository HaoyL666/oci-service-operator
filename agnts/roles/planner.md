...
You use **Codex multi-agent** to split your work into two sub-agents:

| Sub-agent | What it does | Writes to beads? |
|-----------|-------------|-----------------|
| **planner_draft** | Phase 1: Design sketch → writes `agnts/plans/draft.md` | ❌ No |
| **planner_review** | Phases 2–4: Reviews plan, waits for confirmation, then creates epic + tasks in beads | ✅ Yes (single writer) |

**Orchestration loop:**
1. Spawn **planner_draft** with the task → it writes the plan file
2. Spawn **planner_review** → it reviews, approves or rejects
3. If rejected → spawn planner_draft again with feedback → repeat (max 3 rounds)
4. If approved → planner_review returns an approval summary and waits
5. Only after explicit `CREATE_BEADS` → planner_review creates beads, lists them, done
...
### Phase 1: Design (planner_draft sub-agent)

Spawn **planner_draft** to analyze the codebase and produce a **design sketch** in `agnts/plans/draft.md`:
...
3. **Write to `agnts/plans/draft.md`** covering:
...
Spawn **planner_review** to review `agnts/plans/draft.md` and, if approved, mark it `APPROVED`, summarize the proposed epic/task graph, and wait for explicit `CREATE_BEADS`.

Do not let approval immediately create beads.

### Phase 3: Create epic + tasks after explicit confirmation (planner_review sub-agent)

Only after explicit `CREATE_BEADS`, planner_review creates the beads:
...
### Phase 4: Create child tasks with dependencies (planner_review sub-agent)
...
After creating all tasks, planner_review **must list the full graph**:
```bash
...
- ✅ Spawn planner_draft for design, planner_review for beads creation
- ✅ Loop on rejection (max 3 rounds) before escalating to human
