...
`e2e/build-kind-node-image` provides the supported path to build a Kind-compatible custom node image and optionally push it to OCIR for `e2e-lite-local` to consume.
...
- custom Kind node image helper: `e2e/build-kind-node-image`
...
## Custom Kind Node Image Path

If the local machine should not pull the Kind node image from Docker Hub or `mirror.gcr.io`, build it on a builder that can reach upstream artifacts, push it to OCIR, and point `e2e-lite-local` at that OCIR image.

The helper script uses `kind build node-image` with a Kubernetes release by default:

```bash
./e2e/build-kind-node-image
```

That produces a local image named:

```bash
kindest/node:v1.35.0-osok
```

To build and push straight to OCIR:

```bash
OCIR_IMAGE=iad.ocir.io/mytenancy/osok/kindest-node:v1.35.0-osok \
./e2e/build-kind-node-image build-push
```

If Docker Hub access is blocked for the Kind base image pull during the build, mirror the Kind base image first and pass it through `KIND_BASE_IMAGE`:

```bash
KIND_BASE_IMAGE=iad.ocir.io/mytenancy/osok/kindest-base:v20251215-d00590df \
OCIR_IMAGE=iad.ocir.io/mytenancy/osok/kindest-node:v1.35.0-osok \
./e2e/build-kind-node-image build-push
```

After the image is available in OCIR:

```bash
docker login iad.ocir.io
KIND_NODE_IMAGE=iad.ocir.io/mytenancy/osok/kindest-node:v1.35.0-osok \
./e2e/e2e-lite-local
```

Notes:

- This path builds a Kind node image, not an OKE worker-node image.
- `build-kind-node-image` defaults to `KIND_BUILD_TYPE=release` and `KIND_BUILD_SOURCE=v1.35.0`.
- If you need a build from a Kubernetes source checkout or release tarball instead, override `KIND_BUILD_TYPE` and `KIND_BUILD_SOURCE`.
- Publishing the Kind node image to OCIR solves node-image distribution only. OLM image pulls still need working registry access from inside the Kind node.
